...
export { normalizeJid, enhancedDecodeJid, safeGroupOperation, extractUserFromJid };
...
`.trim()
    ...
    reply: {
      ...
      async value(jid, text = '', quoted, options) {
        const result = Buffer.isBuffer(text)
          ...
          : await conn.sendMessage(jid, { ...options, text }, { quoted, ...options })
    ...
    parseMention: {
      /**
       * Enhanced parseMention with @lid support
       * @param {String} text
      ...
      value(text = '') {
        ...
        return [...regularMentions, ...lidMentions]
      },
      enumerable: true,
    },
    getName: {
